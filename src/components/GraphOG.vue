...
export default {
	...
	watch: {
		timeFrame: function(newVal) {
			this.mana()
		}
	},
	methods: {
		mana() {
			this.$socket.client.emit('getKlines', this.timeFrame);
		...
		graph() {
			...
			svg.append('text')
				.attr("class", "symbol")
				.attr("x", 20)
				.text("Operation Bongo III (" + parentThis.timeFrame + ")")
			...
			this.interval = setInterval(() => {
				this.$socket.client.emit('getKlines', parentThis.timeFrame);
	...
	mounted() {
		...
		// this.socket.on('getKlines2', (data) => {
		//     this.data = data;
